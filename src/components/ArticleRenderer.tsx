  ...
  const processedContent = (() => {
    ...
    images.forEach((img) => {
      ...
      if (isSvg) {
        ...
        if (parent) {
          const caption = parent.querySelector(
    ...
    if (!settings.showImages) {
      const captions = tempDiv.querySelectorAll(
        "figcaption, .caption, .image-caption",
      );
      captions.forEach((caption) => {
        ...
        const hasVideo =
          parentFigure &&
          (parentFigure.querySelector("video") ??
            parentFigure.querySelector("iframe"));
    ...
    listItems.forEach((item) => {
      ...
      const hasBlockElements = item.querySelector(
        "p, div, table, blockquote, pre",
      );
      if (hasBlockElements) {
        item.style.marginBottom = "0.75rem";

        const blockElements = item.querySelectorAll(
          "p, div, table, blockquote, pre",
        );
