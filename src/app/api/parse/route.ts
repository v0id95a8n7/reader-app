...
function extractSiteName(html: string, url: string): string | undefined {
  ...
  const twitterSiteMatch =
    /<meta[^>]*name=["']twitter:site["'][^>]*content=["']([^"']+)["'][^>]*>/i.exec(
      html,
    ) ??
    /<meta[^>]*content=["']([^"']+)["'][^>]*name=["']twitter:site["'][^>]*>/i.exec(
      html,
    );
...
function detectVideos(html: string): boolean {
  ...
  const hasYouTubeEmbed =
    /youtube\.com\/embed/i.test(html) || /youtu\.be/i.test(html);
